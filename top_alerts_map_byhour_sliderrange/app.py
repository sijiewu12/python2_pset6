...
app_ui = ui.page_fluid(
    ui.panel_title("Hour Range Selector"),
    ui.input_slider(
        "hour_range",
        "Select Hour Range:",
        min=0,
        max=23,
        value=[8, 17],  # Default selected range (e.g., 8 AM to 5 PM)
        step=1,
        animate=False
    ),
    ui.output_text_verbatim("hour_range_output"),
    ui.input_select(
        id="alert_type",
...
# I ask ChatGPT how to change to code for hour
    @render_altair
    def my_hist():
        ...
        if len(selected) == 3:
            ...
            filtered_data = df_alert_counts[
                ...
                # Check if hour is >= the selected start hour
                (df_alert_counts["hour"] >= input.hour_range()[0]) &
                # Check if hour is <= the selected end hour
